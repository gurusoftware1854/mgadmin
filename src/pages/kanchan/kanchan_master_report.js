...
import Kanchan_user_spinner from "../../components/kanchan_user_spinner.js";
...
const Kanchan_master_report = () => {
  ...
  useEffect(() => {
    ...
    apiClient
      .post(`${baseUrl}/kanchan/admin_kanchan_report.php`, {
        card: selectedUser.id,
      })
      .then((response) => {
        console.log(response.data);
        const transactions = response.data.trans_result || [];
        const loansData = response.data.kanchan_loans_result || [];
  ...
  return (
    <>
      ...
      >
        <HeaderName mainname={"Kanchan"} title={"Master Report"} />
        <div
          style={{
            display: "flex",
            flexDirection: "row",
...
export default Kanchan_master_report;
